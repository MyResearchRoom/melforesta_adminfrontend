...
export default function CategoryList({ onEdit }) {
 ...
 const handleSearchChange = (e) => {
  ...
  try {
    ...
    const res = await axios.delete(`${Base_Url}/api/category/deleteCategory/${categoryId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    toast.success(res.data.message || "category delete success");
    dispatch(fetchCategories(currentPage, itemsPerPage, searchTerm));
  } catch (err) {
    console.error(err);
  ...
  return (
    <div className="px-4 py-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 md:space-x-5">
        <p className="text-lg font-semibold text-left">Total Categories: {totalRecords}</p>
