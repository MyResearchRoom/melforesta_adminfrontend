...
export default function AdminViewProcess() {
  const { id } = useParams();
  ...
  if (!orderData) return <p className="text-red-600">Loading...</p>;
  ...
  const columns = [
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <p className="font-medium text-sm xl:text-base">{capitalizeFirstLetter(row.productName)}</p>
        </div>
      ),
    },
    { label: "Qty", 
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (newStatus === "delivered") {
          setTimeout(() => navigate("/admin/process"), 5000);
        }
      } else {
...
const ProductCard = ({ item }) => (
  ...
  return (
  <section className="space-y-6 px-3 sm:px-5 py-4">
      ...
      { orderData.status !=="cancelled" && (
        nextStage && (
          <div className="flex flex-row space-x-2 items-center text-sm xl:text-base font-medium">
            ...
            >
               {nextStage.replace(/([A-Z])/g, " $1") .replace(/^./, (str) => str.toUpperCase())}
            </button>
          </div>
      ))}
    </div>
